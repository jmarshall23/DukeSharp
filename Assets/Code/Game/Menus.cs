...
public partial class GlobalMembers
{
	...
	*/
	...
	public static void loadpheader(char spot, ref int vn, ref int ln, ref int psk, ref int nump)
	{
		// jmarshall - save game
		/*
				int i;
				const string fn = "game0.sav";
				int fil;
				int bv;
				...
				walock[DefineConstants.MAXTILES - 3] = 255;

				kdfread(bv, 4, 1, fil);
				if (bv != DefineConstants.BYTEVERSION)
				{
				...
				kdfread(nump, sizeof(int), 1, fil);

				kdfread(tempbuf, 19, 1, fil);
				kdfread(vn, sizeof(int), 1, fil);
				kdfread(ln, sizeof(int), 1, fil);
				kdfread(psk, sizeof(int), 1, fil);

				if (waloff[DefineConstants.MAXTILES - 3] == 0)
				{
					allocache(waloff[DefineConstants.MAXTILES - 3], 160 * 100, walock[DefineConstants.MAXTILES - 3]);
				}
				Engine.tilesizx[DefineConstants.MAXTILES - 3] = 100;
				Engine.tilesizy[DefineConstants.MAXTILES - 3] = 160;
				kdfread((string)waloff[DefineConstants.MAXTILES - 3], 160, 100, fil);
	...
	public static int loadplayer(sbyte spot)
	{
		// jmarshall - save game
		/*
				short k;
				...
				string fnptr;
				string scriptptrs = new string(new char[DefineConstants.MAXSCRIPTSIZE]);
				int fil;
				int bv;
				int i;
				int j;
				...
				kdfread(Engine.board.wall[0], sizeof(walltype), DefineConstants.MAXWALLS, fil);
				kdfread(numsectors, 2, 1, fil);
				kdfread(sector[0], sizeof(sectortype), DefineConstants.MAXSECTORS, fil);
				kdfread(Engine.board.sprite[0], sizeof(spritetype), DefineConstants.MAXSPRITES, fil);
				kdfread(headspritesect[0], 2, DefineConstants.MAXSECTORS + 1, fil);
				kdfread(prevspritesect[0], 2, DefineConstants.MAXSPRITES, fil);
				kdfread(nextspritesect[0], 2, DefineConstants.MAXSPRITES, fil);
				kdfread(headspritestat[0], 2, DefineConstants.MAXSTATUS + 1, fil);
				kdfread(prevspritestat[0], 2, DefineConstants.MAXSPRITES, fil);
				kdfread(nextspritestat[0], 2, DefineConstants.MAXSPRITES, fil);
				kdfread(numcyclers, sizeof(numcyclers), 1, fil);
				kdfread(cyclers[0][0], 12, DefineConstants.MAXCYCLERS, fil);
				...
				//ORIGINAL LINE: kdfread(&msx[0],sizeof(int),sizeof(msx)/sizeof(int),fil);
				kdfread(msx[0], sizeof(int), msx.Length, fil);
				//C++ TO C# CONVERTER WARNING: This 'sizeof' ratio was replaced with a direct reference to the array length:
				//ORIGINAL LINE: kdfread(&msy[0],sizeof(int),sizeof(msy)/sizeof(int),fil);
				kdfread(msy[0], sizeof(int), msy.Length, fil);
				...
				kdfread(mirrorEngine.board.wall[0], sizeof(short), 64, fil);
				kdfread(mirrorsector[0], sizeof(short), 64, fil);
				kdfread(show2dsector[0], sizeof(char), DefineConstants.MAXSECTORS >> 3, fil);
				kdfread(actortype[0], sizeof(char), DefineConstants.MAXTILES, fil);
				...
				kdfread(scriptptrs[0], 1, DefineConstants.MAXSCRIPTSIZE, fil);
				kdfread(script[0], 4, DefineConstants.MAXSCRIPTSIZE, fil);
				for (i = 0; i < DefineConstants.MAXSCRIPTSIZE; i++)
				{
					if (scriptptrs[i])
					{
				...
				kdfread(actorscrptr[0], 4, DefineConstants.MAXTILES, fil);
				...
				kdfread(scriptptrs[0], 1, DefineConstants.MAXSPRITES, fil);
				kdfread(hittype[0], sizeof(weaponhit), DefineConstants.MAXSPRITES, fil);
				...
				for (i = 0; i < DefineConstants.MAXSPRITES; i++)
				{
					...
					if ((scriptptrs[i] & 2) != 0)
					{
						hittype[i].temp_data[4] += j;
					}
					if ((scriptptrs[i] & 4) != 0)
					{
						hittype[i].temp_data[5] += j;
					}
				}

				kdfread(lockclock, sizeof(lockclock), 1, fil);
				kdfread(pskybits, sizeof(EXTERN), 1, fil);
				kdfread(pskyoff[0], sizeof(pskyoff[0]), DefineConstants.MAXPSKYTILES, fil);

				kdfread(animatecnt, sizeof(animatecnt), 1, fil);
				kdfread(animatesect[0], 2, DefineConstants.MAXANIMATES, fil);
				kdfread(animateptr[0], 4, DefineConstants.MAXANIMATES, fil);
				for (i = animatecnt - 1; i >= 0; i--)
				{
					animateptr[i] = (int)((int)animateptr[i] + (int)(sector[0]));
				}
				kdfread(animategoal[0], 4, DefineConstants.MAXANIMATES, fil);
				kdfread(animatevel[0], 4, DefineConstants.MAXANIMATES, fil);
				...
				kdfread(randomseed, sizeof(EXTERN), 1, fil);
				kdfread(global_random, sizeof(global_random), 1, fil);
				kdfread(parallaxyscale, sizeof(EXTERN), 1, fil);
				...
				{
					spritesound(DefineConstants.DUKE_JETPACK_IDLE, ps[myconnectindex].i);
				}
				...
				while (k >= 0)
				{
					switch (Engine.board.sprite[k].lotag)
					{
						case 31:
							setinterpolation(ref sector[Engine.board.sprite[k].sectnum].floorz);
							break;
						case 32:
							setinterpolation(ref sector[Engine.board.sprite[k].sectnum].ceilingz);
							break;
						case 25:
							setinterpolation(ref sector[Engine.board.sprite[k].sectnum].floorz);
							setinterpolation(ref sector[Engine.board.sprite[k].sectnum].ceilingz);
				...
				for (i = animatecnt - 1; i >= 0; i--)
				{
					setinterpolation(ref animateptr[i]);
				}
	...
	//C++ TO C# CONVERTER WARNING: The following constructor is declared outside of its associated class:
	public static int saveplayer(sbyte spot)
	{
		// jmarshall - save game
		/*
				int i;
				...
				dfwrite(Engine.board.wall[0], sizeof(walltype), DefineConstants.MAXWALLS, fil);
				dfwrite(numsectors, 2, 1, fil);
				dfwrite(sector[0], sizeof(sectortype), DefineConstants.MAXSECTORS, fil);
				dfwrite(Engine.board.sprite[0], sizeof(spritetype), DefineConstants.MAXSPRITES, fil);
				dfwrite(headspritesect[0], 2, DefineConstants.MAXSECTORS + 1, fil);
				dfwrite(prevspritesect[0], 2, DefineConstants.MAXSPRITES, fil);
				dfwrite(nextspritesect[0], 2, DefineConstants.MAXSPRITES, fil);
				dfwrite(headspritestat[0], 2, DefineConstants.MAXSTATUS + 1, fil);
				dfwrite(prevspritestat[0], 2, DefineConstants.MAXSPRITES, fil);
				dfwrite(nextspritestat[0], 2, DefineConstants.MAXSPRITES, fil);
				dfwrite(numcyclers, sizeof(numcyclers), 1, fil);
				dfwrite(cyclers[0][0], 12, DefineConstants.MAXCYCLERS, fil);
				...
				dfwrite(msx[0], sizeof(int), msx.Length, fil);
				//C++ TO C# CONVERTER WARNING: This 'sizeof' ratio was replaced with a direct reference to the array length:
				//ORIGINAL LINE: dfwrite(&msy[0],sizeof(int),sizeof(msy)/sizeof(int),fil);
				dfwrite(msy[0], sizeof(int), msy.Length, fil);
				...
				dfwrite(actortype[0], sizeof(char), DefineConstants.MAXTILES, fil);
				...
				for (i = 0; i < DefineConstants.MAXSCRIPTSIZE; i++)
				{
					if ((int)script[i] >= (int)(script[0]) && (int)script[i] < (int)(script[DefineConstants.MAXSCRIPTSIZE]))
					{
					...
					{
						scriptptrs = scriptptrs.Substring(0, i);
					}
				}

				dfwrite(scriptptrs[0], 1, DefineConstants.MAXSCRIPTSIZE, fil);
				dfwrite(script[0], 4, DefineConstants.MAXSCRIPTSIZE, fil);

				for (i = 0; i < DefineConstants.MAXSCRIPTSIZE; i++)
				{
					if (scriptptrs[i])
					{
						j = script[i] + (int)&script[0];
						script[i] = j;
					}
				}
				...
				dfwrite(actorscrptr[0], 4, DefineConstants.MAXTILES, fil);
				for (i = 0; i < DefineConstants.MAXTILES; i++)
				{
					if (actorscrptr[i])
					{
				...
				for (i = 0; i < DefineConstants.MAXSPRITES; i++)
				{
					...
					if (actorscrptr[Engine.board.sprite[i].picnum] == 0)
					{
						continue;
					}
					...
					if (hittype[i].temp_data[1] >= j && hittype[i].temp_data[1] < (int)(script[DefineConstants.MAXSCRIPTSIZE]))
					{
						scriptptrs[i] |= 1;
						hittype[i].temp_data[1] -= j;
					}
					if (hittype[i].temp_data[4] >= j && hittype[i].temp_data[4] < (int)(script[DefineConstants.MAXSCRIPTSIZE]))
					{
						scriptptrs[i] |= 2;
						hittype[i].temp_data[4] -= j;
					}
					if (hittype[i].temp_data[5] >= j && hittype[i].temp_data[5] < (int)(script[DefineConstants.MAXSCRIPTSIZE]))
					{
						scriptptrs[i] |= 4;
						hittype[i].temp_data[5] -= j;
					}
				...
				dfwrite(scriptptrs[0], 1, DefineConstants.MAXSPRITES, fil);
				dfwrite(hittype[0], sizeof(weaponhit), DefineConstants.MAXSPRITES, fil);
				...
				dfwrite(pskybits, sizeof(EXTERN), 1, fil);
				dfwrite(pskyoff[0], sizeof(pskyoff[0]), DefineConstants.MAXPSKYTILES, fil);
				dfwrite(animatecnt, sizeof(animatecnt), 1, fil);
				dfwrite(animatesect[0], 2, DefineConstants.MAXANIMATES, fil);
				for (i = animatecnt - 1; i >= 0; i--)
				{
					animateptr[i] = (int)((int)animateptr[i] - (int)(sector[0]));
				}
				dfwrite(animateptr[0], 4, DefineConstants.MAXANIMATES, fil);
				for (i = animatecnt - 1; i >= 0; i--)
				{
					animateptr[i] = (int)((int)animateptr[i] + (int)(sector[0]));
				}
				dfwrite(animategoal[0], 4, DefineConstants.MAXANIMATES, fil);
				dfwrite(animatevel[0], 4, DefineConstants.MAXANIMATES, fil);
				...
				dfwrite(randomseed, sizeof(EXTERN), 1, fil);
				dfwrite(global_random, sizeof(global_random), 1, fil);
				dfwrite(parallaxyscale, sizeof(EXTERN), 1, fil);
	...
	//ORIGINAL LINE: #define LMB (buttonstat&1)
	//C++ TO C# CONVERTER NOTE: The following #define macro was replaced in-line:
	//ORIGINAL LINE: #define RMB (buttonstat&2)
		...
		switch (current_menu)
		{
			...
			case 15000:
				...
				if ((KB_KeyDown[(DefineConstants.sc_Space)] != false) || (KB_KeyDown[(DefineConstants.sc_Return)] != false) || (KB_KeyDown[(DefineConstants.sc_kpad_Enter)] != false) || (KB_KeyDown[(DefineConstants.sc_Y)] != false) || (buttonstat & 1) != 0)
				{
					...
					{
						c = (short)loadplayer((sbyte)lastsavedpos);
						if (c == 0)
						{
							ps[myconnectindex].gm = DefineConstants.MODE_GAME;
						}
